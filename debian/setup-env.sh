...
ExecStartPre=/bin/sh -lc '/usr/bin/${CONTAINER_RUNTIME} ps -aq --filter "label=com.${CONTAINER_RUNTIME}.compose.project=\$COMPOSE_PROJECT_NAME" | xargs -r /usr/bin/${CONTAINER_RUNTIME} rm -f || true'
